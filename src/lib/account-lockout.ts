...
export function recordFailedAttempt(email: string) {
  ...
  rec.count++;
  if (rec.count >= MAX) {
    const d = DURATIONS[Math.min(Math.floor((rec.count - MAX) / MAX), DURATIONS.length - 1)] * 60000;
    rec.lockoutUntil = now + d;
    attempts.set(email, rec);
    return { locked: true, attemptsRemaining: 0, lockoutMinutes: Math.ceil(d / 60000) };
  }
  attempts.set(email, rec);
  return { locked: false, attemptsRemaining: MAX - rec.count };
}
